...
func TestRenderTasksMarkdown(t *testing.T) {
	tf := tasksFile{
		Feature: "demo",
		Waves: []wave{{N: 0, Name: "Foundation", Tasks: []task{
			{ID: "T1", Title: "thing", RequirementRefs: []string{"R1"}, Status: "done"},
			{ID: "T2", Title: "other", Status: "pending"}, // refs vacíos → "—"
		}}},
	}
	...
	for _, want := range []string{"# Tasks — demo", "## Wave 0 — Foundation", "**T1**", "**T2**", "—"} {
...
func TestRenderTasks(t *testing.T) {
	t.Run("stdout -> 0", func(t *testing.T) {
		...
		writeFile(t, filepath.Join(dir, "specforge/features/f/tasks.json"),
			`{"feature":"f","waves":[{"n":0,"tasks":[{"id":"T1","title":"x","status":"done"}]}]}`)
	...
	t.Run("writes tasks.md -> 0", func(t *testing.T) {
		dir := t.TempDir()
		writeFile(t, filepath.Join(dir, "specforge/features/f/tasks.json"), `{"feature":"f","waves":[]}`)
...
func TestCheckTasks(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		tf := tasksFile{Feature: "f", Waves: []wave{{N: 0, Tasks: []task{
	...
	t.Run("duplicate task id + bad status + missing feature", func(t *testing.T) {
		tf := tasksFile{Waves: []wave{{N: 0, Tasks: []task{
