...
func buildGraph(projectDir, featureFilter string) (knowledgeGraph, int) {
	...
	addNode := func(id, typ, label string) {
		...
		seen[id] = true
		nodes = append(nodes, graphNode{ID: id, Type: typ, Label: label})
	}
	var edges []graphEdge
	...
	for _, f := range ff.Features {
		...
		for _, rid := range reqIDs {
			...
			addNode(reqNode, "requirement", rid)
...
// renderGraphMermaid emite un `flowchart LR` estándar. Cada tipo de nodo usa una
// FORMA distinta para leerse de un vistazo:
//
//	feature      ([...])  estadio
...
//	test         {{...}}  hexágono
//
// Las aristas llevan su tipo como etiqueta (-->|tipo|).
func renderGraphMermaid(g knowledgeGraph) string {
	...
	for _, n := range g.Nodes {
		...
		switch n.Type {
		case "feature":
			shape = fmt.Sprintf("%s([\"%s\"])", n.ID, label)
		case "requirement":
			shape = fmt.Sprintf("%s[\"%s\"]", n.ID, label)
		case "code":
			shape = fmt.Sprintf("%s[/\"%s\"/]", n.ID, label)
